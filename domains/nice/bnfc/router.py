"""Router for NICE BNF for Children (BNFc) drug monographs."""
...
router = APIRouter(prefix="/nice/bnfc", tags=[LABEL])
...
@router.post(
    ...
    description="Start a background task that scrapes every drug from the NICE BNFc A-Z listing.",
)
async def scrape_all_bnfc(force: bool = False) -> TaskResponse:
    """Enqueue a full NICE BNFc scrape."""
...
@router.post(
    "/scrape/{slug}",
    response_model=TaskResponse,
    summary="Scrape a single BNFc drug",
    description="Start a background task that scrapes one NICE BNFc drug monograph by slug.",
)
async def scrape_one_bnfc(slug: str) -> TaskResponse:
    """Enqueue a single NICE BNFc drug scrape."""
...
@router.get(
    ...
    description="Return slug and name for every previously scraped BNFc drug monograph.",
)
async def list_bnfc() -> list[ItemSummary]:
    """List all scraped NICE BNFc drug monographs."""
    pool = db.get_pool()
...
@router.get(
    ...
    summary="Get a scraped BNFc drug",
    description="Retrieve full markdown content and metadata for a single BNFc drug monograph.",
)
async def get_bnfc(slug: str) -> ItemContent:
    """Fetch one scraped NICE BNFc drug monograph by slug."""
    pool = db.get_pool()
...
@router.delete("/{slug}", summary="Delete a scraped BNFc drug")
async def delete_bnfc(slug: str) -> dict:
    """Remove a scraped NICE BNFc drug monograph from the database."""
    pool = db.get_pool()
